    ...
    <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.08 } } }} className="space-y-6">
      <motion.div variants={fadeUp} className="flex items-center justify-between">
        <div>
          ...
          <p className="page-description">{mockUsers.length} users in organization</p>
        </div>
      ...
      <motion.div variants={fadeUp} className="grid grid-cols-3 gap-4">
        {[
          { label: 'Admins', count: mockUsers.filter(u => u.role === 'admin').length, icon: Shield, gradient: 'var(--gradient-danger)' },
          { label: 'Managers', count: mockUsers.filter(u => u.role === 'manager').length, icon: UsersIcon, gradient: 'var(--gradient-primary)' },
          { label: 'Employees', count: mockUsers.filter(u => u.role === 'employee').length, icon: UsersIcon, gradient: 'var(--gradient-success)' },
        ].map(stat => (
          <div key={stat.label} className="stat-card">
            <div className="flex items-center justify-between">
              ...
              <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: stat.gradient }}>
      ...
      <motion.div variants={fadeUp} className="table-container">
        <table className="w-full">
          ...
          <tbody>
            {filtered.map(user => (
              <tr key={user.id} className="border-t border-border/50 hover:bg-muted/20 transition-colors">
                ...
                <td className="px-6 py-3.5">
                  <Badge variant="outline" className={`${roleBadge(user.role)} text-xs capitalize`}>{user.role}</Badge>
                </td>
                <td className="px-6 py-3.5">
                  <div className="flex items-center gap-1.5">
                    <div className={`w-1.5 h-1.5 rounded-full ${user.isActive ? 'bg-success' : 'bg-muted-foreground'}`} />
                ...
                <td className="px-6 py-3.5 text-sm text-muted-foreground">{user.createdAt}</td>
